...
if ( ! class_exists( 'BP_Course_Rest_Tracker_Controller' ) ) {
	...
	class BP_Course_Rest_Tracker_Controller{
	    ...
	    function __construct(){
			...
			add_action('wp_insert_comment',array($this,'notes_refresh'),99,2);
	    }
		...
		*/
		...
		/* Notes Refresh */
		function notes_refresh($comment_id,$comment_object){

			if(get_post_type($comment_object->comment_post_ID) == 'unit'){
				

				$this->get_tracker();	

				if(empty($this->tracker['comments']))	{
					$this->tracker['comments'] = array();
				}
				$this->tracker['comments'][$comment_object->comment_post_ID] = time();
