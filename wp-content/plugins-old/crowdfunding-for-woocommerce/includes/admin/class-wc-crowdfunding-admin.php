...
class Alg_WC_Crowdfunding_Admin {
	...
	function output_crowdfunding_report_page() {
			...
			foreach ( $loop->posts as $product_id ) {
				$table_data[] = array(
					...
					date_i18n( get_option( 'date_format' ) . ' ' . get_option( 'time_format' ),
						get_post_meta( $product_id, '_alg_crowdfunding_products_data_updated_time', true ) ),
				);
			}
			$offset += $block_size;
		}
		if ( ! empty( $table_data ) ) {
			...
			$report_html .= alg_get_table_html( array_merge( array( $headings ), $table_data ), array( 'table_class' => 'widefat', 'table_heading_type' => 'horizontal' ) );
		} else {
