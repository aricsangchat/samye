...
if ( ! function_exists( 'alg_calculate_product_orders_data' ) ) {
	...
	function alg_calculate_product_orders_data( $return_value = 'total_orders', $product_id ) {
		...
		$date_query_after = trim( get_post_meta( $product_id, '_' . 'alg_crowdfunding_startdate', true ) . ' ' . get_post_meta( $product_id, '_' . 'alg_crowdfunding_starttime', true ), ' ' );
		$offset           = 0;
		$block_size       = 512;
		while ( true ) {
			$args = array(
		...
		switch ( $return_value ) {
			...
			case 'total_orders':
				return $total_orders;
			default: // 'all'
				return array( 'orders_sum' => $total_sum, 'total_items' => $total_qty, 'total_orders' => $total_orders );
		}
	}
}
...
if ( ! function_exists( 'alg_wc_crowdfunding_calculate_and_update_product_orders_data' ) ) {
	...
	function alg_wc_crowdfunding_calculate_and_update_product_orders_data( $product_id ) {
		$product_orders_data = alg_calculate_product_orders_data( 'all', $product_id );
